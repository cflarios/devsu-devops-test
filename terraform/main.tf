...
resource "digitalocean_kubernetes_cluster" "k8s" {
  ...
  node_pool {
    name       = "devsu" // Node pool name.
    size       = "s-1vcpu-2gb" // Node size.
    tags       = ["k8s", "worker", "devsu"] // Tags to identify resources in DigitalOcean.
...
resource "kubernetes_deployment" "api" {
  ...
  spec {
    ...
    template {
      ...
      spec {
        container {
          image = "cflarios/devsu_devops_assessment:latest" // Docker image for the container.
